...
logger = logging.getLogger("NetVisionWorker.AlertEngine")
...
class AlertEngine:
    ...
    def process_icmp_result(db, device_id, hostname, ip_address, is_online, packet_loss_pct, average_latency):
        """
        Evaluate ICMP ping results and update alerts.
        """
        try:
            if not is_online:
                # Trigger DEVICE_OFFLINE CRITICAL alert
                ...
                # Auto-resolve latency and packet loss alerts if offline
    ...
    def process_tcp_result(db, device_id, hostname, ip_address, port, is_open, status):
        """
        Evaluate TCP port status results and update alerts.
        """
        try:
    ...
    def process_snmp_result(db, device_id, hostname, ip_address, poll_result):
        """
        Evaluate SNMP poll results and update alerts.
        """
        try:
            ...
            # Polling is successful -> resolve SNMP polling failure alert
            ...
            for iface in poll_result.get("interfaces", []):
                ...
                # 3. Traffic threshold check (only if rate calculation is available)
